...
USING DATA FROM HOME RANGE OF EU, N AFRICA, LEVANT
```{r}
sdmdata<-read.csv(file=paste0(genus,"_",species,"_clean.csv"))
complete<-complete.cases(sdmdata)
sdmdata<-sdmdata[complete,]
...
```{r}
gam <- gam(pa ~  Mean_durnal_temp_range	 + 
MaxTemp_WarmestWeek	 + 
Ann_.precip	 + 
Precip_driest_week	 + 
Precip_wettest_week	 + 
...
```{r}
samp1 <- spsample(pol, 50000, type='random', iter=25)
#and get the cell numbers from the raster stack (right to left, up to down)
cells <- cellFromXY(predictors, samp1)
#and transform each of those to the center of its cell.
background_train <- xyFromCell(predictors, cells)
background_sample<-extract(predictors,background_train)
background_complete<-complete.cases(background_sample)
background_train<-background_train[background_complete,]
background_train<-background_train[1:10000,]
  #You'll get a warning saying that your CRS object has lost a comment. This is unimportant and can be ignored.
```
...
```{r}
ecospat.boyce(fit=GAMpreds,pres_test,nclass=0,window.w="default",PEplot = TRUE)
```
...
Error in seq.default(from = mini, to = maxi - window.w, by = (maxi - mini -  : 
...
Error in seq.default(from = mini, to = maxi - window.w, by = (maxi - mini -  : 
  'from' must be a finite number
```{r}
ecospat.boyce(fit=ENSpreds,pres_test,nclass=0,window.w="default",PEplot = TRUE)
```
