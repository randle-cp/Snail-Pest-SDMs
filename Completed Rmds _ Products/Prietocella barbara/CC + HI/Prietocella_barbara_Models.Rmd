...
```{r}
sdmdata<-read.csv(file='C:/Users/kscih/OneDrive/Documents/School/Thesis/Snail_Data/Prietocella_barbara_qGIS_clean.csv')
...
pres_train_data<-pres_train_data[1:nrow(abs_train_data),] #There were less absences than presences, so to prevent further errors in running the code, pres_train_data was reduced to match the number of absences instead of the other way around. The following line of original code is therefore made redundant, but I left it in anyways.
...
```{r}
colnames(train_data)
gam <- gam(pa ~ Mean_Diurnal_Temp_Range + Isothermality + MeanTemp_DriestQ + Precip_Seasonality + Precip_Seasonality + Precip_WarmestQ + Precip_ColdestQ + Lowest_Weekly_Seasonality + Rad_WettestQ + Rad_WarmestQ + Elev + Human_Impact,
